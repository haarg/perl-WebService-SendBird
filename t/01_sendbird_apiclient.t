...
subtest 'Creating API client' => sub {
    my @tests = (
        [{api_token => 1}, qr/^Missing required argument: app_id or api_url/],
        [{app_id => 1}, qr/^Missing required argument: api_token/],
    );
    ...
    ok(WebService::SendBird->new(api_token => 1, app_id => 1), 'Api Client created');
    ok(WebService::SendBird->new(api_token => 1, api_url => 'http://sendbird.com/api/v3'), 'Api Client created');
};
...
subtest 'Making request to api' => sub {
    ...
    my $resp = $api->request(GET => "users/testuser", {extra_param => 1});
    ...
    $resp = $api->request(PUT => "users/testuser", {extra_param => 1});
